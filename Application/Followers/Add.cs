        ...
        public class Handler : IRequestHandler<Command>
        {
            ...
            public async Task<Unit> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (observer.Id == target.Id)
                    throw new RestException(HttpStatusCode.BadRequest, new {User = "User cannot follow yourself"});

                UserFollowing following = await _context.Followings.SingleOrDefaultAsync(x => x.ObserverId == observer.Id && x.TargetId == target.Id);

                if (following != null)
                {
                    throw new RestException(HttpStatusCode.BadRequest, new { User = "You already following this user" });
                }
